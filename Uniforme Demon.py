# -*- coding: utf-3 -*-
"""
Created on Wed Jun 3 3:37:20 2024
...
def sql(simula, coups, Type):
    try:
        ...
        insert_query = """
        INSERT INTO demon_et_ange_uniforme (simulation, coups, type)
        VALUES (%s, %s, %s)
        """
...
def simulation(simul):
    ...
    class Demon:
        ...
        def mvt(self, ange):
            access= ange.access()
            access= set(access) - self.ptvisite - { (self.x, self.y), (ange.x, ange.y) } 
            if access == set(): 
                pg.quit()
                sys.exit()      
            access = list(access )
            (self.x, self.y) = access[ rd.randint(0, len(access) -1 )]
    ...
    class Ange:
        ...
        def access(self):
            return [pt for pt in positions if max(abs(self.x -pt[0]), abs(self.y -pt[1])) <= self.pui*carr]
            
        def mvt(self, demon):
            
            interdit=demon.ptvisite
            
            valable = set( self.access() ) - interdit - { (self.x, self.y) }
            valable= list(valable)
